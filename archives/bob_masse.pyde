...
class Bob(object):
    ...
    def move(self):
        ...
        #self.acceleration.add(self.OtherReaction())
        #self.acceleration.add(self.GroundReaction(grounds))
    ...
    def CheckOtherCollision(self):
        ...
        for other in bobs:
            ...
            if d  < (self.radius+other.radius) and d > 0:
                ...
                f.setMag(map(d, 0, (self.radius+other.radius), RepMag, 0))
                #f.setMag(RepMag*d)
                f *= (other.masse)/(self.masse + other.masse)
                F.add(f)
    ...
    def GroundReaction(self, grounds):
        R = PVector(0,0)
        for ground in grounds:
           if self.intersection(ground):
               F = self.acceleration 
               u = PVector(ground.b.x - ground.a.x, ground.b.y - ground.a.y,).rotate(HALF_PI).normalize()
               R = F.dot(u) * u
        return R
    
    #def Reorientation(self, grounds):
